...
Router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(200).send({
        message: "Congratulations...New course added!",
      ...
      res
        .status(400)
        .send({ message: "Couldn't create the course", error: error.message });
    }
  }
);
...
Router.get("/all-courses", async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send({ message: "Error while fetching", error: error.message });
  }
});
...
Router.post(
  ...
  async (req, res) => {
    try {
      ...
      const uploadedVideo = await cloudinary.uploader.upload(convertedBuffer, {
        resource_type: "video",
        upload_preset: "cloudversity-dev",
      });
      ...
      res
        .status(201)
        .send({ message: "Video uploaded successfully", videoDetails: video });
    } catch (error) {
      console.log("Error occurred while uploading...", error);
      res
        .status(500)
        .send({ message: "Couldn't upload the video", error: error.message });
    }
  }
);
